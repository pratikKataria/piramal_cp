...
import 'package:piramal_channel_partner/ui/dueInvoice/model/invoices_response.dart';
...
class DueInvoicePresenter extends HomePresenter {
  DueInvoiceView _v;
  final tag = "HomePresenter";

  DueInvoicePresenter(this._v) : super(_v);
  ...
  void getInvoices(BuildContext context) async {
    //check for internal token
    if (await AuthUser.getInstance().hasToken()) {
      _v.onError("Token not found");
      return;
    ...
    Dialogs.showLoader(context, "Fetching your Invoices...");
    apiController.post(EndPoints.DueInvoices, body: body, headers: await Utility.header())
      ..then((Response response) async {

        await Dialogs.hideLoader(context);
        InvoicesResponse invoicesResponse = InvoicesResponse.fromJson(response.data);
        ...
        if (invoicesResponse.returnCode) {
          _v.onInvoicesListFetched(invoicesResponse);
        } else {
          _v.onError(invoicesResponse?.message ?? Screens.kErrorTxt);
        }
      })
      ..catchError((e) async {
        await Dialogs.hideLoader(context);
